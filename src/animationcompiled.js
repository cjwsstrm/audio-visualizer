(function() {
  ...
  Sketch.create({
    ...
    setup: function() {
      ...
      for (i = j = 0, ref = NUM_PARTICLES - 1; j <= ref; i = j += 1) {if (window.CP.shouldStopExecution(1)){break;}
        x = random(this.width);
        ...
        try {
          ...
          analyser.onUpdate = (function(_this) {
            return function(bands) {
              ...
              results = [];
              for (k = 0, len = ref1.length; k < len; k++) {if (window.CP.shouldStopExecution(2)){break;}
                particle = ref1[k];
...
window.CP.exitedLoop(2);
    ...
    draw: function() {
      ...
      ref = this.particles;
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {if (window.CP.shouldStopExecution(3)){break;}
        particle = ref[j];
